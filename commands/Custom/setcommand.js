module.exports = {
  desc: "Set a custom command to say whatever you want",
  ...
  execute(message, command) {
    ...
    if (message.member.hasPermission("MANAGE_GUILD")) {
      ...
      if (cmdName in commands) { // Check if command is a bot command
        ...
        util.general.addCommand(message.guild.id, cmdName, command.getArgs().slice(1).join(" "));
        message.channel.send(`Created command **${cmdName}**. Do ${util.general.getServerPrefix(message.guild.id)}setdescription <command> <description> to change the description of the command so that it shows properly in the help menu.`);
      }
    } else {
