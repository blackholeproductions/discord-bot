...
*${util.numberWithCommas(util.xp.getMinXP(level+1)-xp)} xp* until Level ${level+1} (${Math.round((util.xp.getMinXP(level+1)-xp)/xpAvg)} days)
${util.modules.isEnabled("xp-roles", message.guild.id) ? `*${util.numberWithCommas(util.xp.getMinimumXPForRole(message.guild.id, nextHighestRole)-xp)} xp* until <@&${nextHighestRole}> ` : ""} (${Math.round((util.xp.getMinimumXPForRole(message.guild.id, nextHighestRole)-xp)/xpAvg)} days)`);
